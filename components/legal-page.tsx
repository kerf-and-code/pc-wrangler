...
const LEGAL_CSS = `
.sax-legal-page { min-height: 100vh; background: ${C.ink}; padding: 0 20px 80px; }
.sax-legal-wrap { max-width: 760px; margin: 0 auto; }
.sax-legal-brand { display: flex; align-items: baseline; gap: 10px; padding: 26px 0 22px; border-bottom: 1px solid ${C.line}; margin-bottom: 30px; }
.sax-legal-brand .name { font-family: ${SAX.serif}; font-size: 20px; font-weight: 700; color: ${C.text}; }
.sax-legal-brand .tag { font-family: ${SAX.mono}; font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase; color: ${C.muted}; }
.sax-legal-brand a { text-decoration: none; }

.sax-legal { color: ${C.text}; font-family: ${SAX.serif}; line-height: 1.7; font-size: 16px; }
.sax-legal h1 { font-size: 32px; font-weight: 600; letter-spacing: 0.2px; margin: 6px 0 14px; }
.sax-legal h2 { font-size: 20px; font-weight: 600; color: ${C.text}; margin: 34px 0 10px; padding-bottom: 6px; border-bottom: 1px solid ${C.line}; }
...
.sax-legal p { margin: 12px 0; color: ${C.text}; }
.sax-legal strong { color: ${C.text}; font-weight: 600; }
.sax-legal a { color: ${C.plum}; text-decoration: underline; }
.sax-legal ul, .sax-legal ol { padding-left: 22px; margin: 12px 0; }
.sax-legal li { margin: 7px 0; }
.sax-legal hr { border: none; border-top: 1px solid ${C.line}; margin: 30px 0; }
.sax-legal table { width: 100%; border-collapse: collapse; margin: 16px 0; }
.sax-legal th, .sax-legal td { border: 1px solid ${C.line}; padding: 9px 11px; text-align: left; font-size: 14px; vertical-align: top; }
...
.sax-legal blockquote { border-left: 3px solid ${SAX.brass}; background: rgba(26,19,37,0.6); border-radius: 8px; padding: 12px 16px; margin: 18px 0; color: ${C.muted}; }
.sax-legal code { font-family: ${SAX.mono}; font-size: 13px; background: rgba(11,7,18,0.6); border: 1px solid ${C.line}; border-radius: 6px; padding: 1px 6px; color: ${SAX.spark}; }
.sax-legal .meta { color: ${C.muted}; font-size: 14px; margin: 2px 0; }
`;
