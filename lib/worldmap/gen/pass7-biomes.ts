...
const PLAINS = 0, SAVANNA = 1, PRAIRIE = 2, FOREST = 3, TAIGA = 4, RAINFOREST = 5, JUNGLE = 6,
  ...
  SWAMP = 13, BOG = 14, LAKE = 16, SEA = 17, COAST = 18, REEF = 19, MOUNTAINS = 20, VOLCANIC = 21, CANYON = 22;
...
export function pass7Biomes(f: Fields, cfg: GenConfig): void {
  const { width: W, height: H } = f;
  const N = W * H;
  const rndVolcano = passStream(hashSeed(cfg.seed), "volcanic");
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (!f.land[i]) {
      ...
      if (t === 0) f.frozen[i] = 1; // polar sea ice / icebergs
      continue;
    }
    ...
    // 6. Volcanic: a fraction of high ground on a convergent plate boundary.
    if (f.volcanicCandidate[i] && (b === MOUNTAINS || b === ALPINE) && rndVolcano() < 0.12) b = VOLCANIC;
