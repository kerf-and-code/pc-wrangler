  ...
  sections: [
    {
      ...
      blocks: [
        ...
        { kind: "p", text: "Consent comes first, always. Nothing is transcribed until each player has agreed, consent is logged, and the pipeline refuses to run without it. Session audio is deleted automatically 60 days after it is recorded, whether or not anyone asks: the transcript and the moments drawn from it stay, the recording of a person's voice does not." },
        { kind: "p", text: "The name comes from the six axes the disposition model tracks: Voice, Tactics, Arcana, Rapport, Exploration, and Nerve. It reads them for each character, and, once a player has played more than one, for the player underneath them." },
      ],
    },
    {
      id: "tour-gm",
      ...
      blocks: [
        ...
        { kind: "sub", title: "Insight", text: "The analytics. Dispositions shows the six axes per character, what each player said versus how they actually played, and beneath that a read on each PLAYER across every character they have ever run. It refreshes on its own after you finish reviewing a session. Reliability double-codes your transcripts and reports agreement (Cohen's kappa). Dashboard translates everything into plain-language table-health flags. Mechanics is the descriptive dice stats: roll counts, natural 20s and 1s, the d20 distribution, damage dealt, and hit points over the session." },
      ],
    },
    {
      id: "tour-player",
      title: "A tour of the player side",
      blocks: [
        { kind: "p", text: "Players see three groups. Two are about the table they arrived through; one is about them." },
        { kind: "sub", title: "This table", text: "Inventory (the questionnaire that ties their play preferences to this character, and where they set their lines and veils), Journal (their story so far, including how they actually play across the six axes, drawn from logged sessions rather than their own answers), Recaps, Lore (what their character has learned, party-visible entries plus anything you revealed to them personally), Map, and Chat (private party chat, with visibility windows only if they grant them)." },
        { kind: "sub", title: "Session", text: "Schedule (upcoming sessions with RSVP, and the availability polls you post, answered in Discord), Record (the in-person self-recorder), and Check-in (a quick post-session pulse, resubmittable)." },
        { kind: "sub", title: "You", text: "The part that is theirs, not the table's. Campaigns lists every table they play at. Characters is their whole stable across all of them. Threads is a private tracker for favors owed, grudges, and hooks they have not followed up, and nobody else can see it, not even you. Codex gathers what they have learned across every campaign. Your profile is the questionnaire taken AS THEMSELVES rather than as a character. Settings is where they export everything you hold about them, or delete their account." },
      ],
    },
    {
      id: "accounts",
      title: "Guests and accounts",
      blocks: [
        { kind: "p", text: "A player can start as a guest. Claiming a character with the invite link is enough to play: no signup, no password, nothing to remember." },
        { kind: "p", text: "But a guest lives only in that browser. Clear the history and the character goes with it. So the app offers them an account (Discord, Google, or an emailed link), and linking one changes nothing else: same character, same history, same everything. Nothing is reset and nothing is lost. It only makes it durable." },
        { kind: "p", text: "The reason it matters to you is the stable. Once a player has an account, every character they have ever played is one person's, and the model can read the player underneath the characters. A veteran's new PC then starts from how that person tends to play, rather than from nothing." },
      ],
    },
    {
      id: "setup",
      ...
      blocks: [
        { kind: "p", text: "Budget about twenty minutes for you, and about a minute per player. The Start here checklist tracks most of this automatically." },
        { kind: "steps", items: [
          "Create your account and your campaign in the Table workspace. Name the table, pick the system.",
          "Build your cast on the Workspace. Class, subclass, species, and lineage or subrace all come from real lists you pick from, so nothing gets mistyped. If you use third-party content, switch the partner toggles on and it appears in the lists. Anything can be edited later in place, so a half-filled character now is fine.",
          "Send each player their personal invite link from the Roster (the Copy player invite button). Opening it claims their character and, if they want, lets them save it to an account.",
          "Connect Discord: invite the Six Axes bot to your server, then run /setup code:<your share code> in the channel where you want recaps and polls posted. Players run /claim once to link their Discord account to their character, and that is also where they consent to being recorded.",
          "Copy the player setup link from the Roster (the Table Tap card) and pin it in your Discord channel. Any player who rolls on D&D Beyond installs Beyond20 and Six Axes Capture, clicks that link once, and is done. No custom domains, no keeping a tab open.",
          ...
          "Optional but recommended before your first real night: run a short test. Join voice, /record, say a few words, /stop, and have one player roll something. If events show up on Review and a Mechanics row appears, everything is wired.",
        ] },
      ],
    },
    {
      ...
      blocks: [
        { kind: "p", text: "The short version: everyone joins voice, you run /record, you play, you run /stop. Everything after that happens on its own until it needs your judgement." },
        { kind: "steps", items: [
          "Players consent once, when they claim their character. There is no consent prompt at record time and no tab to keep open: if they have the capture extension, their rolls just arrive.",
          "Everyone joins the voice channel, then you run /record. The bot joins, captures each speaker on their own track, opens a session, and links you as the narrator. Rolls made on D&D Beyond flow in alongside, attributed to each character. To record into a specific session number, add session:2.",
          "Play. If someone new joins voice mid-session, nothing breaks; long sessions are captured in rotating chunks automatically.",
          "When you wrap, run /stop. The bot uploads per-speaker audio, and then transcription and event extraction start on their own. You do not have to press anything.",
          "Playing in person instead? Skip /record: each player opens the session link on their phone and uses the self-recorder on the Record page, or you upload per-player audio on the Capture page. Same pipeline from there.",
        ] },
        { kind: "sub", title: "Somebody does not want to be recorded", text: "That is fine and it does not stop the session. On the Capture page you can opt any character out of any session; their track is excluded from transcription and from analysis, and everyone else records normally. A player can also withdraw at any time by telling you, and they are excluded from that session onward." },
      ],
    },
    {
      id: "insights",
      ...
      blocks: [
        { kind: "p", text: "After a session the raw material becomes readable in this order. Only one step needs you." },
        { kind: "steps", items: [
          "Transcription starts on its own after /stop. It is consent-gated: if someone at the table has neither consented nor been opted out, the job stops and tells you so rather than recording them anyway.",
          "Extraction follows automatically, and the proposed events are usually waiting by the time you look.",
          "Review them. This is the human checkpoint, and it is the only one. Approve, edit, or reject each; use the confidence threshold to accept the obvious ones in a click. While you are here, Accept + create turns any captured NPC, place, or faction into a Codex entry.",
          "When you decide the last proposal, the session finishes itself. The recap is drafted, and the disposition model refreshes. There is no Done button to remember, and if you deliberately want to leave some proposals undecided, a button appears to let you.",
          "Edit the drafted recap on the Session Log, then send it: Email and Post to Discord are separate buttons over the same draft, so whatever you see on screen is exactly what goes out.",
          "Open Insight, then Mechanics for the dice story: who rolled, the d20 distribution, crits and fumbles, damage by type, and each character's hit points.",
    ...
    {
      id: "dispositions",
      title: "Reading the dispositions",
      blocks: [
        { kind: "p", text: "Every character gets a read across the six axes, and it comes in two parts. The prior is what the player said about themselves in the questionnaire. The posterior is how they actually played, drawn from the events you approved. The gap between them is the interesting part, and the model does not assume the questionnaire is right: it estimates how much self-report predicts behaviour, per axis, from your data." },
        { kind: "sub", title: "The player behind the characters", text: "Once someone has played more than one character, the model also reads THEM: how that person tends to play, pooled across everything they have run. A new character then starts shrunk toward that rather than toward nothing, and sharpens as it earns its own evidence." },
        { kind: "sub", title: "What players see, and what they do not", text: "A player always sees their own character's posterior on their Journal. They do NOT see the read on themselves unless you share it. That one is a claim about the person rather than the character, so it is yours to offer: on Insight, then Dispositions, each player has a Share with them button. It is reversible, and they still choose whether to look." },
        { kind: "sub", title: "Read the intervals, not just the number", text: "A posterior built on one session with no questionnaire is mostly the model's prior wearing a number. Where that is true, the app says so in plain language and draws the uncertainty band under the score. Wide band, thin evidence. Do not build a conversation with a player on a wide band." },
      ],
    },
    {
      id: "build",
    ...
    {
      id: "privacy",
      title: "Recording, consent, and what happens to it",
      blocks: [
        { kind: "p", text: "You are recording your friends' voices. The app takes that seriously and so should you. This is the whole of it, in plain terms." },
        { kind: "sub", title: "Consent is once, not every session", text: "A player consents when they claim their character, and it stands for the campaign. They never see a consent prompt at record time, because being asked mid-game in front of everyone is not a free choice." },
        { kind: "sub", title: "The pipeline enforces it", text: "Transcription will not run if someone present has neither consented nor been opted out. This is not a warning you can click past: the job stops. If it does, the Capture page tells you who." },
        { kind: "sub", title: "Audio is deleted after 60 days", text: "Automatically, whether or not anyone asks, and nobody can extend it. The transcript and the moments drawn from it stay, because that is the campaign's record. The recording of a person's voice does not. Old moments will show as expired rather than playing, and that is the promise working." },
        { kind: "sub", title: "Players own their data", text: "On their Settings page, any player can export everything you hold about them, or delete their account. Deleting removes their recordings, their transcribed words, their questionnaire answers, and their notes. Their characters stay in your campaign with the personal link severed, because the story your table told together is yours as well as theirs." },
        { kind: "sub", title: "Your responsibility as GM", text: "Recording law varies, and some places require every person to agree. Say out loud when recording starts, especially on Discord, where the bot joining is the only signal. Be careful with minors: a parent or guardian should consent." },
      ],
    },
    {
      id: "action",
    ...
    },
    {
      ...
      blocks: [
        { kind: "sub", title: "A player's rolls are not arriving", text: "Check three things in this order. Is a session actually open? Rolls are dropped if nothing is live, and that looks exactly like a broken extension. Do they have BOTH Beyond20 and Six Axes Capture installed? And did they click your /x/ setup link, which is what tells the extension which campaign to send to?" },
        { kind: "sub", title: "A player's first roll shows as unlinked", text: "That is expected, and it is not lost. Rolls attach to a character only once that character's D&D Beyond id is known, and the first roll is what teaches it. The extension popup grows a Link your character picker; the player chooses once, and every earlier roll is backfilled and every later one attributes on its own. Tell your players this in advance, or they will assume it failed." },
        ...
        { kind: "sub", title: "Transcription will not start", text: "That is the consent gate working. Someone at that table has neither consented nor been opted out. Fix it on the Capture page: opt them out for that session, or have them tap I consent in Discord." },
        ...
        { kind: "sub", title: "A character seems to be missing from party coverage", text: "It has no subclass recorded, or a subclass the catalog does not recognise. The Workspace roster flags them and gives you a fix link. A level 1 or 2 character with no subclass is fine and is not flagged, because it genuinely does not have one yet." },
        { kind: "sub", title: "An old moment will not play", text: "The audio was deleted under the 60-day retention policy. The transcript and the event are still there; the recording is not, and cannot be brought back. That is the policy working, not a fault." },
      ],
    },
  ],
};
